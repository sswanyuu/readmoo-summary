...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadSummaries() {
    try {
      ...
      if (summaries.length === 0) {
        summariesList.innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">No saved summaries yet</div>';
        return;
      }
      ...
      summariesList.innerHTML = '<div style="padding: 20px; text-align: center; color: #ef4444;">Failed to load summaries</div>';
    }
  }

  // Clear all summaries
  async function clearSummaries() {
    if (!confirm('Are you sure you want to delete all saved summaries? This action cannot be undone.')) {
      return;
    }
  ...
  function openHelp(e) {
    e.preventDefault();
    chrome.tabs.create({ url: 'https://github.com/your-repo/readmoo-summary#help' });
  }

  // Open privacy policy
  function openPrivacy(e) {
    e.preventDefault();
    chrome.tabs.create({ url: 'https://github.com/your-repo/readmoo-summary#privacy' });
  }
